...
void w25qReadPage(uint8_t *page, uint16_t pagenum) {
    ...
    HAL_GPIO_WritePin(_W25QXX_CS_GPIO,_W25QXX_CS_PIN,GPIO_PIN_RESET); //W25Q_CS_LO();

    HAL_SPI_Transmit(&hspi2, com, 5, 10);

    HAL_SPI_Receive(&hspi2, page, 256, 10);
    HAL_GPIO_WritePin(_W25QXX_CS_GPIO,_W25QXX_CS_PIN,GPIO_PIN_SET); //W25Q_CS_HI();
}
...
void w25qWritePage(uint8_t *page, uint16_t pagenum) {
   ...
   // while(w25qxx.Lock==1)
    ...
    HAL_SPI_Transmit(&hspi2, page, 256, 10);
     HAL_GPIO_WritePin(_W25QXX_CS_GPIO,_W25QXX_CS_PIN,GPIO_PIN_SET); //W25Q_CS_HI();

    W25qxx_WaitForWriteEnd(); //w25qWaitForReady();
...
void w25qWritingByUSB(uint16_t dpagenum, uint8_t *bufByUSB) {
    ...
    W25qxx_EraseSector(current_sector_addr);//w25qEraseSector(current_sector_addr);
