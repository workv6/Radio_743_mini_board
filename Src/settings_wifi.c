...
static const GUI_WIDGET_CREATE_INFO _wfsCreate[] = {
  { FRAMEWIN_CreateIndirect, "Выбор WIFI", ID_FRAMEWIN_0, 5, 75, 705, 270, 0, 0x0, 0 },
...
static void _cbwfs(WM_MESSAGE * pMsg) {
 ...
 int z;
                ...
                case WM_TIMER: // событие таймера
                   /* sprintf(tbuf,"wifi.rssi\r\n");
                    printf2(tbuf,11);
                       ...
                       if (rssi>70) sprintf(temp_string,"плохой");
                       else if (rssi>60) sprintf(temp_string,"слабый");
                       else if (rssi>50) sprintf(temp_string,"хороший");
                       else sprintf(temp_string,"отличный");
                       ...
                       sprintf(tbuf,"Уровень сигнала %s",temp_string);
  ...
  case WM_INIT_DIALOG:
    ...
    FRAMEWIN_SetText       (hItem, " Выбор сети WIFI ");



                       hItem = WM_GetDialogItem(pMsg->hWin, ID_TEXT_0);
                       TEXT_SetFont(hItem, &GUI_FontVerdana32);
                       sprintf(tbuf,"WIFI: %s, пароль: %s",ssidNames[0],wifiPass[0]);
                       TEXT_SetText(hItem,tbuf);

                       hItem = WM_GetDialogItem(pMsg->hWin, ID_TEXT_1);
                       TEXT_SetFont(hItem, &GUI_FontVerdana32);
                       sprintf(tbuf,"WIFI: %s, пароль: %s",ssidNames[1],wifiPass[1]);
                       TEXT_SetText(hItem,tbuf);

                       hItem = WM_GetDialogItem(pMsg->hWin, ID_TEXT_2);
                       TEXT_SetFont(hItem, &GUI_FontVerdana32);
                       sprintf(tbuf,"WIFI: %s, пароль: %s",ssidNames[2],wifiPass[2]);
                       TEXT_SetText(hItem,tbuf);

                       hItem = WM_GetDialogItem(pMsg->hWin, ID_TEXT_3);
                       TEXT_SetFont(hItem, &GUI_FontVerdana32);
                       sprintf(tbuf,"WIFI: %s, пароль: %s",ssidNames[3],wifiPass[3]);
                       TEXT_SetText(hItem,tbuf);

                       hItem = WM_GetDialogItem(pMsg->hWin, ID_TEXT_4);
                       TEXT_SetFont(hItem, &GUI_FontVerdana32);
                       sprintf(tbuf,"WIFI: %s пароль: %s",ssidNames[4],wifiPass[4]);
                       TEXT_SetText(hItem,tbuf);
